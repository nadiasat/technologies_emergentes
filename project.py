...
# if type not 1 or 2 end if type is 1 then movie if type is 2 then serie
...
# only keeps the line where Genres contains genre to lower case
df = df.filter(df["Genres"].contains(genre.lower()))

# show only title and all genres and idmbp rating only if there is some datas
if df.count() > 0:
